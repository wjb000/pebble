...
function usePreparedStl(url: string, mode: 'bbox' | 'flange' | 'zonly' = 'bbox') {
  const raw = useLoader(STLLoader, url) as BufferGeometry
  return useMemo(() => {
    if (mode === 'flange') return footOnBottomFlange(raw)
    if (mode === 'zonly') {
      const g = raw.clone()
      g.computeBoundingBox()
      const b = g.boundingBox
      if (b) g.translate(0, 0, -b.min.z)
      g.computeBoundingBox()
      g.computeVertexNormals()
      return g
    }
    return footGeometry(raw)
  }, [raw, mode])
}
...
export function WheeledChassis({
  ...
  const torsoGeom = usePreparedStl(TORSO_STL, 'zonly')
