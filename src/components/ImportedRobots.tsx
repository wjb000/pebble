/**
 * Twin CAD — XLeRobot without the IKEA RÅSKOG cart body.
 * Wheeled drive base + dual SO-101 + OG XLe head (Vector-Wangel, Apache-2.0).
 */
...
/** Drop arm/head mounts from cart-top height onto the wheeled deck (no RÅSKOG shelves). */
const DECK_DROP_M = 0.4
const SKIP_MESH = /raskogbody\.stl/i
...
/** Show wheeled base + both arm trees + OG head; never the IKEA cart body. */
function showDeckTwin(robot: URDFRobot) {
  for (const link of Object.values(robot.links)) link.visible = true
  robot.traverse((obj) => {
    const n = obj.name.toLowerCase()
    if (SKIP_MESH.test(n) || n.includes('raskogbody')) {
      obj.visible = false
      return
    }
    obj.visible = true
...
/** Arms/head are authored on the cart top — lower them onto the wheel deck. */
...
function useUrdf(url: string, workingPath: string) {
  ...
  useEffect(() => {
    ...
    // Show the robot as soon as the URDF parses; bump again when meshes finish.
    manager.onLoad = bump
    manager.onError = (res) => {
    ...
    loader.loadMeshCb = (path, mgr, material, done) => {
      // Skip the heavy IKEA RÅSKOG cart body STL (mobile + visual).
      if (SKIP_MESH.test(path)) {
        done(new Object3D())
        return
      }
...
export function meshAttribution(kit: KitBuild) {
  return (
    `${kitCaption(kit)}. ` +
    `XLeRobot (Vector-Wangel): wheeled base + dual SO-101 + OG head pan/tilt (no IKEA RÅSKOG cart body), Apache-2.0. ` +
